...
def run_loso_analysis_with_weighting(target_col):
    """
    Performs a full Leave-One-Site-Out cross-validation for a given target,
    applying sample weights to prioritize extreme values.
    """
    ...
    for test_site in sites:
        ...
        # --- Create sample weights based on deviation from the mean ---
        train_mean = y_train.mean()
        sample_weights = np.abs(y_train - train_mean)
        
        # Normalize weights to have a mean of 1 for stability
        if sample_weights.sum() > 0:
            sample_weights = sample_weights / sample_weights.mean()
        else: # Handle case where all target values are the same
            sample_weights = np.ones(len(y_train))

        # Initialize and train the CatBoost model with sample weights
        ...
        # Pass the calculated weights to the fit method
    ...
    results_csv_path = os.path.join(out_path, f'catboost_results_{target_col}_weighted.csv')
    predictions_csv_path = os.path.join(out_path, f'catboost_predictions_{target_col}_weighted.csv')
    ...
    print(f"\n--- Pooled Metrics for {target_col.upper()} (Weighted) ---")
    ...
    if not all_preds_df.empty:
        ...
        for site in all_preds_df["Site"].unique():
            ...
            ax.set_title(f"Observed vs. Predicted {target_col} (Weighted) for Site: {site}")
            ax.legend()
            ax.grid(True)
            fig.autofmt_xdate()
            ...
            plot_filename = f'catboost_{target_col}_{site}_timeseries_weighted.png'
    ...
    model_filename = f'{target_col}_weighted.json'
    
    # Calculate weights for the full dataset for the final model
    full_mean = y.mean()
    final_weights = np.abs(y - full_mean)
    ...
    final_model.save_model(os.path.join(final_model_path, model_filename))
    print(f"Final weighted model for {target_col} saved to {os.path.join(final_model_path, model_filename)}")
...
if __name__ == '__main__':
    ...
    for target in targets_to_run:
        ...
        run_loso_analysis_with_weighting(target_col=target)
