...
def safe_r2(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """R², but returns NaN if variance of y_true is zero or too few samples."""
...
def run_loso_analysis(target_col: str):
    ...
    - Filters expert_flag_ch4 != 4 (if present)
    - Training rows are restricted to sites listed in methane_towers.csv
      (test set is always the left-out site, regardless of tower list)
    - Saves per-site plots, pooled metrics/plot, and a final model trained on tower-restricted rows
    """
    ...
    # Ensure site_reference exists and is string
    ...
    # Filter out flagged methane rows
    if 'expert_flag_ch4' in df.columns:
        ...
        print("Note: 'expert_flag_ch4' column not found; no expert-flag filter applied.")

    # Require valid target and site_reference
    ...
    for test_site in sites_all:
        ...
        # Train mask: other sites, but only towers
        ...
        r2   = safe_r2(y_test, y_pred)  # robust to zero-variance
    ...
    # Pooled metrics (all left-out predictions pooled)
    ...
    # ---- 9) Final model on tower-allowed rows ----
