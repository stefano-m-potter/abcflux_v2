...
def run_loso_analysis(target_col):
    ...
    WITHOUT using land_cover as a predictor. Trains on all but one site, predicts the
    left-out site, aggregates results, saves per-site time series plots, pooled CSVs,
    a final model, and an Observed vs Predicted scatter plot to *_nolc* outputs.
    """
    # 1. Load and prepare dataset
    print(f"--- Processing Target (no land cover): {target_col.upper()} ---")
    file_path = "/explore/nobackup/people/spotter5/anna_v/v2/v2_model_training_final.csv"
    df = pd.read_csv(file_path)
    # land_cover is intentionally NOT used; do not cast/use it
    ...
    # 3. Define predictors and categorical features (NO land_cover)
    feature_cols = [
        ...
        'snow_cover', 'snow_depth', 'NDSI_snow_cover'
    ]
    categorical_features = ['month']  # only month is categorical now

    # Drop rows where the current target or site_reference is missing
    df = df.dropna(subset=['site_reference', target_col])

    # 4. Define output paths (send to *_nolc* dirs)
    base_out = "/explore/nobackup/people/spotter5/anna_v/v2"
    loocv_out_path = os.path.join(base_out, "loocv_nolc", target_col)
    figures_path = os.path.join(loocv_out_path, "figures")
    models_out_path = os.path.join(base_out, "models_nolc")
    ...
    for test_site in sites:
        ...
        model = CatBoostRegressor(
            ...
            l2_leaf_reg=0.1,
            rsm=0.8,
            cat_features=categorical_features,  # only 'month'
            verbose=0,
    ...
    results_csv_path = os.path.join(loocv_out_path, f'catboost_results_{target_col}_nolc.csv')
    predictions_csv_path = os.path.join(loocv_out_path, f'catboost_predictions_{target_col}_nolc.csv')
    ...
    print(f"\n  --- Pooled Metrics (NO land cover) for {target_col.upper()} ---")
    print(f"  Pooled R²: {r2_all:.4f}, Pooled RMSE: {rmse_all:.4f}, Pooled MAE: {mae_all:.4f}")

    # Summary metrics across sites
    mean_r2 = results_df['R2'].mean();   median_r2 = results_df['R2'].median()
    mean_rmse = results_df['RMSE'].mean(); median_rmse = results_df['RMSE'].median()
    mean_mae = results_df['MAE'].mean(); median_mae = results_df['MAE'].median()
    print(f"\n  --- Summary Across Sites (NO land cover) for {target_col.upper()} ---")
    ...
    # 8. Plotting: per-site time series
    ...
    for site in all_preds_df["Site"].unique():
        ...
        ax.set_title(f"Observed vs. Predicted {target_col} (NO land cover) — Site: {site}")
        ...
        plot_filename = f'catboost_{target_col}_{site}_timeseries_nolc.png'
        plot_path = os.path.join(figures_path, plot_filename)
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.close(fig)
    print(f"  All site plots saved to: {figures_path}")

    # 8b. NEW: Pooled Observed vs Predicted density plot across all left-out sites
    ...
    if not pooled.empty:
        ...
        # Density (hexbin) plot – darker red = higher density
        hb = ax.hexbin(
            pooled['Observed'],
            pooled['Predicted'],
            gridsize=80,               # adjust density resolution if needed
            cmap='Reds',
            bins='log',                # log color scale highlights high-density areas
            mincnt=1
        )
        cbar = fig.colorbar(hb, ax=ax)
        cbar.set_label('log10(N points)')

        # 1:1 reference line in solid black
        ...
        # Round pooled metrics to 2 decimals for annotation
        annot = f"R² = {r2_all:.2f}\nRMSE = {rmse_all:.2f}\nMAE = {mae_all:.2f}"
        ax.text(
        ...
        pooled_plot_path = os.path.join(
            figures_path, f'catboost_{target_col}_obs_vs_pred_all_sites_cat.png'
        )
    ...
    print("\n  Training and saving final model on all data (NO land cover)...")
    ...
    final_model.fit(X, y)
    model_filename = os.path.join(models_out_path, f'{target_col}_nolc.json')
...
if __name__ == '__main__':
    ...
    for target in targets_to_run:
        print(f"\n{'='*50}\nRUNNING ANALYSIS (NO LAND COVER) FOR: {target.upper()}\n{'='*50}")
        run_loso_analysis(target_col=target)
        print(f"\n{'='*50}\nCOMPLETED ANALYSIS (NO LAND COVER) FOR: {target.upper()}\n{'='*50}")
